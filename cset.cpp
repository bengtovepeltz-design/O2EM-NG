/*
------------------------------------------------------------------------------

O2EM-NG
Modern SDL3 Edition

Character ROM data for the Philips Videopac G7000 /
Magnavox Odyssey² family of home video game systems.

Based on the original O2EM project.
http://o2em.sourceforge.net

Original O2EM

    Created by
        Daniel Boris <dboris@comcast.net>
        Copyright (c) 1997–1998

    Developed by
        Andre de la Rocha <adlroc@users.sourceforge.net>
        Arlindo M. de Oliveira <dgtec@users.sourceforge.net>

This file contains the original character ROM data used by
the Videopac/Odyssey² Video Display Controller (VDC).

Preserved in O2EM-NG for compatibility with the original
O2EM emulation core.

------------------------------------------------------------------------------
*/

#include "types.h"
#include "cset.h"


Byte cset[512]= {
};

